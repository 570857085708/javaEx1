...
//		, 아닐 경우 '여자 입니다'
	...
	String str[] = inputNum.split("-"); // 문자열 분리(split)
	phone1 = str[0];
	phone2 = str[1];
	phone3 = str[2];

	phone1 = inputNum.substring(0,3); // (substring)
	phone2 = inputNum.substring(4,8);
	phone3 = inputNum.substring(9,13);
	
	System.out.println("주민 번호 입력 : ");
	String inputJumin = s.next();
	char genderStr = inputJumin.charAt(7);  // (charAt)
	String genderStr2 = inputJumin.substring(7,8); // (substring)
		if(genderStr2.equals("1") || genderStr2.equals("3")) {
			System.out.println("남자");  // 문자열비교 (equals) 
	}	else {
